...
export function RecipeDetailPage({ publisher, slug }: { publisher: string; slug: string }) {
  ...
  return (
    <main className="detail-page" itemScope itemType="https://schema.org/SoftwareApplication">
      ...
      <section className="boundary"><h2>Immutable payload references</h2>{recipe.source?.recipe_url ? <><p>Recipe source</p><a href={recipe.source.recipe_url}><code>sha256:{recipe.content_sha256}</code></a></> : null}<p>Build source</p>{recipe.source?.bundle_url ? <a href={recipe.source.bundle_url}><code>Source bundle</code></a> : recipe.build?.context?.sha256 ? <a href={`/v1/source-bundles/${recipe.build.context.sha256}`}><code>sha256:{recipe.build.context.sha256}</code></a> : <span>Not declared</span>}<p>Dockerfile</p><code>{recipe.build?.dockerfile || "Not declared"}</code><p>Weights</p>{recipe.artifacts?.map((artifact, index) => <code key={`${artifact.repository}/${artifact.revision}/${artifact.download_bytes}/${index}`}>{artifact.repository}@{artifact.revision} · {artifact.download_bytes} bytes</code>)}</section>
