...
export function RecipesPage({ fixedPublisher }: { fixedPublisher?: string } = {}) {
  ...
  const filtered = useMemo(
    () => sortRecipes(available.filter((recipe) => recipeMatches(recipe, filters)), filters.sort),
    [available, filters],
  );
    ...
    <main className="catalog-page">
      ...
      {catalog ? <div className="catalog-browser">
        ...
        <section className="catalog-results" aria-label="Recipe results">
          <div className="catalog-results-heading"><p aria-live="polite"><strong>{filtered.length}</strong> of {available.length} recipes</p><label><span>Sort by</span><select aria-label="Sort recipes" value={filters.sort} onChange={(event) => update("sort", event.target.value as RecipeSort)}><option value="catalog">Catalog order</option><option value="model">Model A–Z</option><option value="sparks">Fewest Sparks</option><option value="download">Smallest download</option></select></label></div>
          ...
          <div className="recipe-grid">{pageItems.map((recipe) => <RecipeCard key={`${recipe.publisher}/${recipe.slug}`} recipe={recipe} />)}</div>
