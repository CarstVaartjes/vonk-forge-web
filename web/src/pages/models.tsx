...
export function ModelsPage() {
  ...
  return <main className="models-page">
    ...
    {models && visible.length ? <div className="model-list">{visible.map((model) => { const capabilities = modelCapabilities(model); const version = model.versions[0]; return <article className="model-card" key={`${model.publisher}/${model.slug}`}><div className="model-card-heading"><div><p className="eyebrow">{model.family ? `Family · ${model.family}` : "Model"}</p><h2><a href={`/models/${model.publisher}/${model.slug}`}>{model.title}</a></h2><code>{model.publisher}/{model.slug}</code></div><span className="model-recipe-count">{model.versions.length} {model.versions.length === 1 ? "version" : "versions"}</span></div>{version ? <dl className="model-card-facts" aria-label="Model release details"><div><dt>Version</dt><dd>{version.version}</dd></div><div><dt>Variant</dt><dd>{version.variant || "Not declared"}</dd></div><div><dt>Access</dt><dd>{accessLabel(version)}</dd></div></dl> : null}<ul className="model-tags" aria-label="Declared capabilities and model tags">{capabilities.slice(0, 3).map((capability) => <li key={capability}>{filterLabel(capability)}</li>)}{!capabilities.length ? <li>Capability evidence unknown</li> : null}</ul><div className="model-card-footer"><a href={`/recipes?q=${encodeURIComponent(model.title)}`}>{model.recipe_count} {model.recipe_count === 1 ? "public recipe" : "public recipes"}</a><a className="button primary" href={`/models/${model.publisher}/${model.slug}`}>View versions</a></div></article>; })}</div> : null}
